...
if py3k:
    ...
    #Fixed Collections to collections.abc
...
class lazy_attribute(object):
    ...
    def __init__(self, func,name=None):
        functools.update_wrapper(self, func, updated=[])
        self.getter = func
        self.__name__ = name or func.name
...
def run(app=None, server='wsgiref', host='127.0.0.1', port=8080,
    ...
    try:
        ...
        if not server.quiet:
            _stderr("Graphite online....\n")
            _stderr(f"Linked with http://{server.host}:{server.port}/\n")
            _stderr("Starting data server...\n")
            #Need my custom tags so changed
